...
export default function PortfolioSummary() {
  ...
  useEffect(() => {
    async function loadPerson() {
      ...
      try {
        const allPeople = await base44.entities.Salesperson.list();
  ...
  if (person.experience?.length > 0) {
    pages.push(
      <div key="experience" className="slide bg-slate-50 flex flex-col p-12 overflow-hidden">
          ...
          {person.experience.slice(0, 3).map((exp, i) => (
            <div key={i} className="bg-white rounded-2xl p-5 shadow-sm">
              ...
              {exp.achievements?.slice(0, 2).map((achievement, j) => (
                <p key={j} className="text-slate-600 text-sm mt-2 flex items-start gap-2 line-clamp-2">
  ...
  if (person.case_study) {
    pages.push(
      <div key="casestudy" className="slide bg-white flex flex-col p-12 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto w-full space-y-4">
          <div className="grid grid-cols-3 gap-4 bg-slate-50 rounded-xl p-4">
            <div>
              ...
              <div className="text-slate-900 text-sm line-clamp-4">{person.case_study.challenge}</div>
            </div>
            <div>
              <div className="text-xs text-slate-500 uppercase mb-1">Solution</div>
              <div className="text-slate-900 text-sm line-clamp-4">{person.case_study.solution}</div>
            </div>
            <div>
              <div className="text-xs text-slate-500 uppercase mb-1">Role</div>
              <div className="text-slate-900 text-sm line-clamp-4">{person.case_study.role}</div>
            </div>
          </div>
  ...
  // Page 5: Skills - Limit to fit on page
  if (person.skills?.length > 0) {
    pages.push(
      <div key="skills" className="slide bg-slate-50 flex flex-col items-center justify-center p-12 overflow-hidden">
        ...
        <div className="flex flex-wrap justify-center gap-3 max-w-4xl">
          {person.skills.slice(0, 12).map((skill, i) => (
            <span key={i} className="bg-amber-500 text-slate-900 px-5 py-2 rounded-full font-semibold text-sm">
              {skill}
            </span>
          ))}
        </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {person.achievements.slice(0, 4).map((achievement, i) => (
                <div key={i} className="bg-white rounded-xl p-3 flex items-start gap-2">
                  <span className="text-amber-500 flex-shrink-0">★</span>
                  <span className="text-slate-700 text-sm line-clamp-2">{achievement}</span>
                </div>
              ))}
            </div>
          </div>
  ...
  // Page 6: 30-60-90 Day Plan - Only show if has content
  const hasDayPlanContent = person.day_plan && (
    person.day_plan.day_30?.items?.length > 0 ||
    person.day_plan.day_60?.items?.length > 0 ||
    person.day_plan.day_90?.items?.length > 0
  );
  
  if (hasDayPlanContent) {
    pages.push(
      <div key="dayplan" className="slide bg-white flex flex-col items-center justify-center p-12 overflow-hidden">
        ...
        <div className="grid grid-cols-3 gap-6 max-w-5xl w-full">
          {['day_30', 'day_60', 'day_90'].map((key, i) => {
            ...
            if (!plan || !plan.items?.length) return <div key={key} />;
            return (
